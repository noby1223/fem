import getfem as gf
import postprocess_3d_ver2 as post_3d
import pandas_3d_tp as pd3
import wet_conductibity_tp as w_cond
import msh_mf_point_tp as msh_mf
import add_region_tp as add_re
...
m = gf.Mesh("import", "gmsh", "/home/noby/fem/fem3d/TPウェット塗膜抵抗測定/msh_geo/TP測定_231012_wet.msh")


r_p = paint_region = 15

bd1 = m.region(13)#電極のregion
...
for i in range(200):
    ...
    V = md[i].variable("V")
    mf.export_to_vtk(
        "fem/fem3d/TPウェット塗膜抵抗測定/vtk/fem_3d_wet_electric_potential_TPwetteikou_" + str(i) +'.vtk' , "ascii", V, "Electric potential"
    )
    ...
    I_hairetu = post_3d.postprocess_3d(m, V, conductivity,  bd_wsf2, pts_junban,conductivity_wet,bd2)
    ...
    pd3.excel_3d(r_p,i,m,mf, I_sekisan, I_hairetu,ws,bd_ws,bd2)
...
pd3.excel_3d_matome(cond_wet_mtm,I_hai_mtm,V_matome,area_hairetu,I_seki_mtm,bd2,bd_ws,region_w)
