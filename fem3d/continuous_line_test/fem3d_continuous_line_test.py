import getfem as gf
import postprocess_3d_ver3_continuous as post_3d_con
import numpy as np
import pandas_3d_continuous as pd3
import wet_conductibity_continuous as w_cond
import msh_mf_point as msh_mf
import add_region_continuous as add_re
...
for i in range(10):
    ...
    bd1 = m.region(1)#電極のregion
    bd2 = m.region(3)#paintのregionを追加
    ...
    ANODE = 1#m.regions()でregionを確認してから代入しよう
    PAINT = 2
    BODY_SURFACE = 12 #ここでTPの表裏、計7面を分けてregionを設定し、Dhiriclet_condition_multiでやろうとしたら謎のエラーがでた。（０Vが1000Vになったりして）わかるのに結構時間かかってしまった。
    WET_SURFACE = 4
    ...
    if i == 0 : 
        ...
        conductivity_wet = [100]*len(bd_ws[0]) #1000μS/cm　がﾒｰﾄﾙ換算で100mS/ｍかなと
    ...
    V = md[i].variable("V")
    mf.export_to_vtk(
        "fem/fem3d/continuous_line_test/vtk/fem3d_continuousline_test_" + str(i) +'.vtk' , "ascii", V, "Electric potential"
    )
    ...
    #m と　mfのポイント合わせ
    pts_junban = msh_mf.point_awase_continuous(m,mf,i)
    


    I_hairetu = post_3d_con.postprocess_3d(m, V, conductivity,  bd_wsf1, pts_junban,conductivity_wet,i)
