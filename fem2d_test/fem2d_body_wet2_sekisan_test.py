...
import postprocess_2d_test as post_2d
import numpy as np
import pandas_2d_test as pd2
...
I_hai_mtm = []
cvnb_a = m.nbcvs()#全凸の数
cvnb_p = m.nbcvs()- len(m.region(34)[0])#paintの凸数
...
for i in range(10):
    ...
    for w in m.region(34)[0]:  # WET塗膜の凸毎にsetregionで設定していく
        ...
        if i == 0:
            ...
            if 1/np.linalg.norm(I_sekisan[w+115]) ** 1.5 > 1:#元ファイルではｗのみだったが、ずれてしまうので115加算する
                c_cd = 1

            else:
                c_cd = 1 / np.linalg.norm(I_sekisan[w+115]) ** 1.5

        conductivity_wet.append(c_cd)
    ...
    mf.export_to_vtk(
        "fem/fem2d_test/vtk/fem_2d_wet_electric_potential" + str(i) + ".vtk", mf, V, "Electric potential"
    )
    

    I_hairetu = post_2d.postprocess_2d_test(m, V, mf,conductivity,conductivity_wet)
    ...
    pd2.excel_2d_test(i,m,mf, I_sekisan, I_hairetu)
...
pd2.excel_2d_matome_test(cond_wet_mtm,I_hai_mtm)
