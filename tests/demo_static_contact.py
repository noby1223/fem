...
if version == 1 or version == 2:  # defining the matrices BN and BT by hand
    ...
    if version == 3:
        ...
        md.add_initialized_data("friction_coeff", [friction_coeff])
        md.add_nodal_contact_with_rigid_obstacle_brick(
            mim, "u", "lambda_n", "lambda_t", "r", "friction_coeff", GAMMAC, obstacle, 1
        )
    ...
    for ii in range(100):
        ...
        sol = gf.linsolve_superlu(
            ...
            gf.asm_integral_contact_Uzawa_projection(
                GAMMAC, mim_friction, mfu, U, mflambda_partial, lambda_n, mfd, OBS, r
            ),
        )
...
mfd.export_to_vtk(
    "static_contact.vtk", "ascii", mfvm, VM, "Von Mises Stress", mfu, U, "Displacement"
)
