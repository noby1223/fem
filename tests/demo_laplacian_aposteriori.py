...
for refiter in range(5):
    ...
    edgeresidual = (
        "0.25*element_size*sqr(%s)*(Test_psi + Interpolate(Test_psi,neighbour_elt))"
        % grad_jump
    )

    ETA1tmp = gf.asm(
        "generic", mim, 1, bulkresidual, -1, md, "psi", 1, mfP0, np.zeros(mfP0.nbdof())
    )
    ...
    mfu.export_to_vtk(
        "laplacian%d.vtk" % refiter, mfu, U, "u", mfP0, ETA1, "eta1", mfP0, ETA2, "eta2"
    )
