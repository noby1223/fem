...
for step in range(1, nbstep + 1):
    ...
    if w < 1:
        ...
        RT1 = axrot_matrix([0, h * 0.75, 0], [0, h * 0.75, 1], (2 - w) * dtheta)
        RT2 = axrot_matrix([0, 0, 0], [0, 1, 0], w * dtheta2)
        RB1 = axrot_matrix([0, h * 0.25, 0], [0, h * 0.25, 1], -(2 - w) * dtheta)
        RB2 = RT2.transpose()
    else:
    ...
    # Nonlinear solve
    md.solve("very noisy", "max_iter", 50, "max_res", 1e-7, "lsearch", "simplest")
